  ...
  {/* <ItemDescriptionCombobox
              ...
              <tbody>
                  {items.map((item, index) =>  {
                      const descriptionKey:any = `description${69 + (index > 2 ? index + 1 : index)}`;
  console.log(descriptionKey,"key")
                  return(
                    <tr key={item.id} className={item.isDiscount ? 'bg-gray-50' : ''}>
                      <td className="p-1">
                        {item.isDiscount ? (
                          ...
                          <ItemDescriptionCombobox
                          // ref={refs.description69??index===0}
                          // onKeyDown={(e) => onKeyDown(e, 'description69')??index===0}
                          value={item.description}
                          onChange={(value) => handleItemChange(item.id, value)}
                          onItemSelect={(selectedItem) => handleItemSelect(item.id, selectedItem)}
                        />
                    ...
                    <td className="p-1">
                      {!item.isDiscount && (
                        <QuantityInput
                        ref={index===0?refs.description69:refs.description71}
                          value={item.quantity}
                          onChange={(value) => handleQuantityChange(item.id, value)}
                          onBlur={() => handleQuantityBlur(item.id, item.quantity)}
                          onKeyDown={index===0?(e) => onKeyDown(e, 'description69'):(e) => onKeyDown(e, 'description71')}
                        />
                      )}
                    </td>
                    <td className="p-1">
                      <CurrencyInput
                        ref={index===0?refs.description70:refs.description73}
                        value={item.price}
                        onChange={(value) => handlePriceChange(item.id, value)}
                        onKeyDown={index===0?(e) => onKeyDown(e, 'description70'):(e) => onKeyDown(e, 'description73')}
                      />
                    </td>
                    <td className="p-1">
                      <CurrencyInput
