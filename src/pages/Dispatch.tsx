...
function Dispatch() {
  ...
  const tableStyles = {
    ...
    resizeHandle:
      "absolute top-0 right-0 w-4 h-full cursor-col-resize hover:bg-blue-500 z-10",
    input:
  ...
  const handleDriverAssignment = async (
    ...
    try {
      ...
      const { error: towmastError } = await supabase
        ...
        .eq(
          "dispnum",
          towRecords.find((r) => r.id === selectedRow?.id)?.towmast.dispnum
        );
    ...
    async (page: number) => {
      ...
      try {
        ...
        if (error) {
          ...
          const dispatchedTrue = allData
            .filter(
              (item: any) =>
                item.towmast?.dispatched === true ||
                item.towmast?.dispatched === "true"
            )
            .sort((a: any, b: any) => {
              const aNum = Number(a.towmast?.dispnum) || 0;
              const bNum = Number(b.towmast?.dispnum) || 0;
              return bNum - aNum; // Descending
            });

          // 2. Dispatched FALSE → sort by priority ASC then dispnum ASC
          const dispatchedFalse = allData
            .filter(
              (item: any) =>
                !(
                  item.towmast?.dispatched === true ||
            ...
            .sort((a: any, b: any) => {
              const aPriority = Number(a.towmast?.priority) || 0;
              const bPriority = Number(b.towmast?.priority) || 0;
              if (aPriority !== bPriority) {
                return aPriority - bPriority;
              }
              const aDisp = Number(a.towmast?.dispnum) || 0;
              const bDisp = Number(b.towmast?.dispnum) || 0;
              return aDisp - bDisp;
            });

          // 3. Combine
          const sortedData = [...dispatchedTrue, ...dispatchedFalse];
          ...
          <div className="flex flex-col w-full gap-2">
            <div className="grid grid-cols-3 gap-2 w-full">
              ...
              <select className="relative bg-gray-50 text-xs flex items-center justify-center space-x-1 px-2 py-1 border rounded hover:bg-gray-50 text-black transition w-full">
                ...
                <option className="Without appointments">
        ...
        <table className={tableStyles.table} style={{ tableLayout: "fixed" }}>
          <colgroup>
            {orderedFields.map((field) => (
              <col
                key={field}
                style={{ width: `${columnWidths[field] || 100}px` }}
              />
            ))}
          </colgroup>
          <thead>
                ...
                >
                  ...
                  <div
                    className={`${tableStyles.resizeHandle} ${
                      isResizing && resizingRef.current?.column === field
                        ? "bg-blue-500"
                        : ""
                    }`}
                    ...
                    style={{
                      ...
                      backgroundColor:
                        selectedRow?.id === record.id
                          ? "#bfdbfe"
                          : getRowStyle(record)?.backgroundColor,
                    }}
